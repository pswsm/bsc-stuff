...
def fetch_html(session: requests.Session, orig_url: str, folder: str = "", warc_folder: str = "warcs"):
    ...
    if is_live:
        try:
            session.get(f"{protocol}{orig_url}{folder}", timeout=5)
...
if __name__ == "__main__":
    ...
    the_session: requests.Session = requests.Session()
    for the_url in the_urls:
        html: str = get_html(the_url)
        fetch_html(the_session, the_url, warc_folder="tests/warc_utils")
        links: set[str] = filter_urls(the_url, get_links(html))
        for link in links:
            print(link)
            if re.search(r"[\w]\.cat", link, flags=re.UNICODE):
                fetch_html(the_session, link, warc_folder="tests/warc_utils")
            else:
                fetch_html(the_session, the_url, folder=link, warc_folder="tests/warc_utils")
